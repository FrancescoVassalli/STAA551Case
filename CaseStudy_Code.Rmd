...
toyota.df %<>% select(-c('Mfg_Month', 'Mfg_Year', 'Cylinders', 'Fuel_Type')) 
...
summary(powerTransform(cbind(KM, Age, HP, QuartTax, Weight) ~ 1, toyota.df))
...
df.factors <- df.factors %>% select(Price.log, Age, KM.sqrt, Weight, HP.cube, 
...
fit.noperiod <- lm(Price.log ~ ., data = df.factors[,-8])
anova(fit.noperiod)
...
mod.df <- toyota.df %>% 
  ...
  select(Price.log, Age, KM.sqrt, Weight, HP.cube, QuartTax.cube, CC, Period, everything())
...
mod2.df <- mod.df %>% 
  select(-c("Doors", "Metallic", "Diesel"))
...
fit_final <- 
  lm(Price.log ~ 
       Age + KM.sqrt +  Weight + HP.cube + QuartTax.cube + CC + Guarantee + BOVAG 
     , data=toyota.df)
summary(fit_final)
