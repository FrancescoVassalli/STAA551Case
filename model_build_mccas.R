...
fit0 <- lm(Price ~ Age + KM + Weight + HP + QuartTax)
plot(fit0) # def no
...
#par(mfrow=c(3,2))
#par(mar=c(4, 4, 2, 0.5))
plot(Age,resid5,xlab="Age",ylab="residual") #alright, maybe not totally const var
abline(a=0,b=0)
plot(KM,resid5,xlab="KM",ylab="residual") #reasonably const var
abline(a=0,b=0)
plot(Weight,resid5,xlab="Weight",ylab="residual") # some concern on the right (outliers)
abline(a=0,b=0)
plot(HP,resid5,xlab="HP",ylab="residual") # some concern on the right (outliers)
abline(a=0,b=0)
...
toyota.df$Weight.trans <- Weight^-4 #don't know this one is wise -> could be the convergence problem in powerTransform
attach(toyota.df)
...
fit6.5 <- lm(log(Price) ~ Age + KM + Weight + HP.trans + QuartTax + Fuel_Type)  
summary(fit6.5)


fit7 <- lm(log(Price) ~ Age + KM + Weight + HP.trans + QuartTax + Fuel_Type + Guarantee)  
summary(fit7) 
plot(fit7)
vif(fit7) #fuel_type VIF high

fit8 <- lm(log(Price) ~ Age + KM + Weight + HP.trans + QuartTax + Guarantee)  
summary(fit8)  # diagnostics on this one look pretty good
vif(fit8)

fit9 <- lm(log(Price) ~ Age + KM + Weight + HP.trans + Fuel_Type + Guarantee)  
...
fit10 <- lm(log(Price) ~ Age + KM + Weight + HP.trans + QuartTax + CC + Fuel_Type2 + QuartTax:Fuel_Type2 + Guarantee)  
summary(fit10)
vif(fit10)
par(mfrow=c(2,2))
plot(fit10) #qq tails are an issue, std resids don't look ideal, but it's not horrible
